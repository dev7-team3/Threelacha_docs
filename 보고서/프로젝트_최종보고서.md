...
>   Glue Catalog와 Athena 등 생소한 AWS 서비스 도입에 앞서 로컬에 유사 환경을 선제적으로 구축함으로써, 전체 아키텍처의 데이터 흐름을 명확히 이해하고 설계 오류를 사전에 차단하기 위함.
...
>
> [상세내용: Lambda를 통한 인스턴스 기동 자동화](https://www.notion.so/lambda-2e06e9180a9681be8843e7a4419a5051?pvs=21)

#### 🔑 Key Point: Athena 쿼리 지연 문제로 인한 RDS 도입

> 💡 Athena 쿼리로 S3 데이터를 가져오는 경우와 OLTP DB인 PostgreSQL을 사용하였을 때의 데이터 조회 속도 비교를 통해 RDS 도입 진행.
>
> [상세내용: RDS 도입 배경 - Athena 쿼리 지연 문제](https://www.notion.so/RDS-Athena-2e06e9180a96812d93caf1292517ff5c?pvs=21)
...
[상세내용: 데이터 파이프라인 단계별 상세 정의](https://www.notion.so/2e06e9180a96814bad7ff578e6ce450f?pvs=21)
...
>
> [상세내용: t3.medium 환경에서의 Airflow 운영 안정화 및 리소스 최적화 전략](https://www.notion.so/t3-medium-Airflow-2e06e9180a9681d8984fe81fc130a3db?pvs=21)
...
>
> [상세내용: DooD 환경의 Airflow-dbt 컨테이너 간 Docker 소켓 권한 장애](https://www.notion.so/DooD-Airflow-dbt-Docker-2e06e9180a968127b01ad0b74030cec0?pvs=21)
...
>
> [상세내용: Parquet 내부 컬럼과 파티션 메타데이터 간 정합성 오류 해결](https://www.notion.so/Parquet-2e06e9180a9681d58d41f1453e9a5f03?pvs=21)
...
>
> [상세내용: Athena 워크그룹 설정 충돌에 따른 dbt 모델 S3 저장 경로(location) 미적용 문제](https://www.notion.so/Athena-dbt-S3-location-2e06e9180a968139a5bfecbcfe05b198?pvs=21)
